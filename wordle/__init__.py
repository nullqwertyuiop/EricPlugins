...
@dispatch(
    Twilight(
        [
            PrefixMatch(),
            FullMatch("wordle"),
            ArgumentMatch("-s", "--statistic", action="store_true", optional=False)
            @ "_",
        ]
    )
)
...
@decorate(*decorators)
async def wordle_keyboard_cfg(
    app: Ariadne, event: MessageEvent, no_keyboard: ArgResult, keyboard: ArgResult
):
    ...
    if no_keyboard and keyboard:
        ...
        return
    wordle_group_cfg: WordleGroupConfig = module_create(
        WordleGroupConfig, event.sender.group
    )
    if no_keyboard:
